...
body {
 background-image:url("disneyplus-clone/original.jfif");
...
.row {
  background: linear-gradient( 163.91deg, rgba(217, 217, 217, 0.4), rgba(217, 217, 217, 0.1) );
box-shadow: 0 4px 40px -10px rgba(0, 0, 0, 0.2);
...
<body>
	<div class="row">
      <form class="row g-3 needs-validation" action="UserServlet?page=changePassword" method="POST" novalidate>
                          <input type="hidden" name="id" value="${user.uid}"> 
		<h1>Enter New Password</h1>
                <!--<form action="UserServlet?page=verifyUsername" method="post">-->
		<div class="form-group">
                    			<p><label for="username">Password</label></p>

			<input type="password"   pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
  title="Must contain at least one  number and one uppercase and lowercase letter, and at least 8 or more characters" required
 name="password" id="user_email">
			<button style="color:white;"onclick="showSpinner()">Reset Password</button>
                        </form>
		</div>
		<div class="footer">
			<h5>New here? <a href="#">Sign Up.</a></h5>
			<h5>Already have an account? <a href="#">Sign In.</a></h5>
			<p class="information-text"><span class="symbols" style="color: white;" title="">&hearts; </span><a href="https://www.facebook.com/adedokunyinka.enoch" target="_blank" title="Connect with me on Facebook"></a></p>
		</div>
